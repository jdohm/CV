...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \draw (9,-.75) coordinate(lsb);
  \draw (7,-1.5) node[and port](or0){};
  \draw (9,-1.5) node[and port](or1){};
  \draw (7,-2.5) node[and port](and1){};
  \draw (7,-3.5) node[and port](and2){};
  \draw (or0.bout) node[right]{} (or0.bin 1) node[ocirc,left]{}(or0.bin 2)node[ocirc,left]{};
  \draw (and1.bout) node[ocirc,right]{};
  \draw (and1.bin 1) node[left]{};
  \draw (and2.bout) node[ocirc,right]{};
  \draw (and2.bin 1) node[ocirc,left]{};
  ...
  (and2.out) - - +(2.5,0)coordinate(and2Out);
  \draw (and0Out) |- (a)
%  (and0Out) |- (d)
  (b) -| +(-.5,-2) node[rground](gnd){}
  (cnt0) |- (lsb) |- (c)
  (or1.out) |- (d)

  (and1Out) |- (e)
  (and2Out) |- (g)
  (f) -| +(-0.25,2) node[vcc]{} node[left]{VCC};
